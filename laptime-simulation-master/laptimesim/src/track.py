...
class Track(object):
    ...
    def __init__(self, pars_track: dict, parfilepath: str, trackfilepath: str, vel_lim_glob: float = np.inf,
        ...
        track_name = self.pars_track["trackname"]
        if track_name in pars_track_tmp:
            self.pars_track.update(pars_track_tmp[track_name])
        else:
            print(f"WARNING: Track '{track_name}' not found in configs. Using defaults.")
            self.pars_track.update({
                "s12": 2000.0, "s23": 4000.0, "mu_mean": 1.0,
                "drs1_act": 0.0, "drs1_deact": 0.0, "drs2_act": 0.0, "drs2_deact": 0.0,
                "pitspeed": 22.222, "pit_in": 0.0, "pit_out": 0.0
            })
